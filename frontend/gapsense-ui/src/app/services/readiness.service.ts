...
export class ReadinessService {
  ...
  createQuestion(question: Partial<Question> & { moduleId?: string; topicId?: string }): Observable<Question> {
    ...
    const directTopicId = question.topicId || null;
    const topicName = (question as Partial<Question>).topic?.trim() || '';

    const buildBody = (topicId: string | null) => ({
      title: question.title,
      questionText: question.questionText,
      questionType: question.questionType,
      difficulty: question.difficulty,
      moduleId, // GUID from modules API
      topicId,
    ...
    // If topicId isn't provided, resolve it using the topic name (from free-text input).
    if (!directTopicId && topicName) {
      return this.getTopicList(moduleId).pipe(
        switchMap((topics) => {
          const found = topics.find((t) => t.topicName.trim() === topicName);
          const topicId = found?.id ?? null;
          const body = buildBody(topicId);
          return this.http.post<ApiResponse<Question>>(`${this.apiUrl}/questions`, body).pipe(map((res) => res.data));
        })
      );
    }

    const body = buildBody(directTopicId);
    return this.http.post<ApiResponse<Question>>(`${this.apiUrl}/questions`, body).pipe(map((res) => res.data));
  }

  // update a question
  updateQuestion(id: string, updates: Partial<Question> & { topicId?: string | null }): Observable<Question> {
    const directTopicId = updates.topicId ?? null;
    const topicName = updates.topic?.trim() || '';

    const moduleId = (updates as any).moduleId || '';
    ...
    if (!directTopicId && topicName && moduleId) {
      return this.getTopicList(moduleId).pipe(
        switchMap((topics) => {
          const found = topics.find((t) => t.topicName.trim() === topicName);
          const topicId = found?.id ?? null;
          const body = buildBody(topicId);
          return this.http.put<ApiResponse<Question>>(`${this.apiUrl}/questions/${id}`, body).pipe(map((res) => res.data));
        })
      );
    }
  ...
  submitQuiz(
    ...
    const profile = this.authUi.currentUser();
    ...
    const initials = (name: string): string => {
      ...
      return (first + second).toUpperCase().slice(0, 5);
    };
    ...
    const body = {
      ...
      studentId,
      studentName,
      studentAvatar: initials(studentName),
