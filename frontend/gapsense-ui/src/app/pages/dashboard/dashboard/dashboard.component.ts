...
@Component({
  ...
  imports: [NgClass, RouterLink, TopBarComponent, SidebarComponent],
  template: `
    <app-top-bar />
    <app-sidebar />
    ...
    <div class="w-full space-y-10 pb-12">
      ...
      <!-- KPI Grid — minimal monochrome: black icons & values, grey labels; red badge text only on -4.2% -->
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="relative lg:col-span-2 rounded-2xl border border-slate-100 bg-white p-8 pb-20 shadow-sm">
          ...
          <a
            ...
            [attr.aria-label]="quickLinkLabel()"
          >
            <span class="material-symbols-outlined text-2xl">{{ quickLinkIcon() }}</span>
          </a>
        </div>
      </div>
    </div>
