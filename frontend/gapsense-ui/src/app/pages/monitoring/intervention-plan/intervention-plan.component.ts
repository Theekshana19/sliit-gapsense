              ...
              <section>
                ...
                <div class="space-y-5">
                  <div class="flex flex-col gap-1.5">
                    <label class="text-sm font-semibold text-slate-700" for="courseCode">Module code</label>
                    <input
                      id="courseCode"
                      type="text"
                    ...
                    @if (showError('courseCode')) {
                      <p class="text-xs font-medium text-rose-600">Enter a module code.</p>
                    }
                  </div>

                  <div class="flex flex-col gap-1.5">
                    <label class="text-sm font-semibold text-slate-700" for="studentRef">Student user id (GUID)</label>
                    <input
                      id="studentRef"
                      type="text"
                      formControlName="studentRef"
                      autocomplete="off"
                      placeholder="Paste the student's AspNetUsers Id (GUID)"
                    ...
                    @if (showError('studentRef')) {
                      <p class="text-xs font-medium text-rose-600">Enter a valid student user GUID.</p>
                    }
                  </div>
...
export class InterventionPlanPageComponent {
  ...
  readonly form = this.fb.nonNullable.group({
    courseCode: ['', [Validators.required, Validators.minLength(3), Validators.maxLength(32)]],
    studentRef: [
      '',
      [Validators.required, Validators.pattern(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i)],
    ],
  ...
  async saveDraft(): Promise<void> {
    ...
    this.toast.show('Follow-up saved to the database.', 'success');
    this.applyPublishValidators();
    this.mode = 'publish';
  ...
  private async persistPlan(): Promise<boolean> {
    ...
    const moduleCode = v.courseCode.trim().toUpperCase();
    const notesLines = [
      moduleCode ? `Module: ${moduleCode}` : null,
      `Risk: ${v.riskGroup}`,
      v.interventionType ? `Type: ${v.interventionType}` : null,
      v.dueDate ? `Due: ${v.dueDate}` : null,
      actions ? `Notes: ${actions}` : null,
    ].filter((line): line is string => !!line);
    const notes = notesLines.join('\n');

    return await this.monitoring.createPlan({
