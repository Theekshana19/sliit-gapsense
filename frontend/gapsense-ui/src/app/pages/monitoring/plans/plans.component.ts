    ...
    <div class="mx-auto w-full max-w-7xl space-y-6 pb-8">
          ...
          <table class="min-w-full text-sm">
            <thead class="bg-slate-50 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
              <tr>
                ...
                <th class="px-4 py-3">Student (user id)</th>
            ...
            <tbody class="divide-y divide-slate-100">
              @for (plan of filteredPlans; track plan.id) {
                <tr>
                  ...
                  <td class="px-4 py-4 font-mono text-xs text-slate-700">{{ plan.studentRef }}</td>
      ...
      <div class="grid gap-4 lg:grid-cols-2">
        <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
          ...
          <p class="mt-2 text-4xl font-bold text-slate-900">{{ successRate }}%</p>
          <p class="mt-3 text-sm text-slate-600">Share of interventions marked completed (closed) in the database.</p>
          <div class="mt-4 h-3 w-full rounded-full bg-slate-100">
            <div class="h-3 rounded-full bg-indigo-700" [style.width.%]="successRateNumber"></div>
          </div>
        </div>
        <div class="rounded-2xl border-l-4 border-rose-600 bg-white p-6 shadow-sm ring-1 ring-slate-200">
          ...
          <p class="mt-2 text-4xl font-bold text-rose-700">{{ pendingCount }} open</p>
          <ul class="mt-4 space-y-2 text-sm text-slate-700">
            <li>{{ highRiskCount }} High-risk plans (UI tag; stored in notes when creating)</li>
            <li>{{ overdueCount }} Open interventions older than their scheduled date (from notes)</li>
          </ul>
        </div>
      </div>
    </div>
...
export class MonitoringPlansPageComponent implements OnInit {
  ...
  private async loadPlans(): Promise<void> {
    this.plans = await this.monitoring.fetchPlans();
  }
  ...
  get successRateNumber(): number {
    return Number(this.successRate);
  }
  ...
  get pendingCount(): number {
    return Math.max(0, this.totalCount - this.completedCount);
  }

  get highRiskCount(): number {
    return this.plans.filter((p) => p.riskGroup === 'high').length;
  }

  get overdueCount(): number {
    const today = new Date().toISOString().slice(0, 10);
    return this.plans.filter((p) => p.status !== 'completed' && p.dueDate && p.dueDate < today).length;
  }
  ...
  async onRemove(id: string): Promise<void> {
    void id;
    this.toast.show('Deleting interventions is not supported by the API yet.', 'info');
  }
}
