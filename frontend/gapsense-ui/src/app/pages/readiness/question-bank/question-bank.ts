...
export class QuestionBankComponent implements OnInit {
  ...
  ngOnInit() {
    this.readinessService.getModuleList().subscribe((mods) => {
      // backend filters accept either module name or module code; we use moduleName for display
      this.modules = mods.map((m) => m.moduleName);
    });

    this.readinessService.getTopicList().subscribe((topics) => {
